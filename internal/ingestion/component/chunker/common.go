...
// matching exactly the text the decorator keys on after finalizeGeneralChunks
// runs removeTag and chunkOutputs materializes context. Routing every chunk id
// through this one function — instead of recomputing the text at each
// consumption point — guarantees the streamed crop-upload key and the
// decorator's ck["id"] can never diverge, including for image/table chunks
// whose text still carries position tags when there is no media context.
